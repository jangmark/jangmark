...
from nobel n 
where (`year` between 1910 and 2010) and category ='Peace' and fullname is not null order by `year` asc;
...
select *
from nobel n 
where category not in ('Physiology or Medicine') order by fullname asc; 
...
-- 13) 수장자의 출신 국가가 ‘Europe’, ‘North America’, 공백이 아닌 모든 정보 출력
...
select birth_country, count(*) as sb 
from nobel n 
group by  birth_country 
having sb>=10
order by sb desc;
